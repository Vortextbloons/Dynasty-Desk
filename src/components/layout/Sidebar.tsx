...
const navItems: NavItem[] = [
  { to: '/dashboard', label: 'Dashboard', icon: LayoutDashboard, group: 'Front Office' },
  { to: '/roster', label: 'Roster', icon: Users, group: 'Front Office' },
  { to: '/lineup', label: 'Lineup', icon: ClipboardList, group: 'Front Office' },
  ...
  { to: '/contracts', label: 'Contracts', icon: FileSignature, group: 'League' },
...
export function Sidebar() {
  ...
  const grouped = groups.map((g) => ({ group: g, items: navItems.filter((i) => i.group === g) }))
    ...
    <aside className="hidden md:flex md:w-64 lg:w-72 shrink-0 flex-col border-r border-[var(--color-line-soft)] bg-[var(--color-surface-1)]/80 backdrop-blur">
      <div className="px-6 pt-6 pb-5 border-b border-[var(--color-line-soft)]">
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="font-display text-lg leading-none">DYNASTY DESK</div>
                        ...
                        <>
                          <span
                            className={cn(
                              ...
                              isActive ? 'bg-[var(--color-primary)]' : 'bg-transparent group-hover:bg-[var(--color-line-strong)]',
                            )}
                          />
